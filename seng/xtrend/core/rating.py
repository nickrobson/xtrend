...
def get_rating(ric):
    try:
        return CompanyRating.objects.filter(ric = ric, date = currentDate, schema = SCHEMA_VERSION).get().rating
    except:
        rating = calculate_rating(ric)
        model = CompanyRating(ric = ric, date = currentDate, rating = rating, schema = SCHEMA_VERSION)
        model.save()
        return rating

def calculate_stock_rating(ric):
    logger.info('    Calculating Stock Rating for', ric)
...
def calculate_news_rating(ric):
    logger.info('    Calculating News Rating for', ric)
...
def calculate_rating(ric):
    logger.info('Calculating Overall Rating for', ric)
